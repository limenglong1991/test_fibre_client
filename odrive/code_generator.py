    ...
    for item in json:
        ...
        if 'type' in item:
            if item['type'] in {'int8', 'uint8', 'int16', 'uint16', 'int32', 'uint32', 'int64', 'uint64'}:
                item['type'] += '_t'
                is_property = True
            elif item['type'] in {'bool', 'float', 'double'}:
                is_property = True
            elif item['type'] in {'function'}:
                item['type'] = 'void'
                item['name'] = prefix + item['name']
                flat_list.append(item)
                flat_list = flat_list + get_flat_endpoint_list(item['inputs'] + item['outputs'], item['name'] + '.', id_offset)
            elif item['type'] in {'json', 'object'}:
                is_property = False
            else:
                item['type'] += '_t'
                is_property = True
